...
class MultiAgentMushrBlockEnv(MujocoEnv, utils.EzPickle):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.action_space = Box(low=np.array([-0.33, 0, -0.33, 0]), high=np.array([0.33, 1.2, 0.33, 1.2]), dtype=np.float64)
    ...
    def set_init_states(self, init_states):
        ...
        car2_pos - states[6:8]
        car2_quats - states[8:12]
        block_pos - states[12:14]
        block_quats - states[14:18]
        
        '''
        qpos = self.unwrapped.data.qpos.flatten()
        qvel = self.unwrapped.data.qvel.flatten()
        qpos[0:2] = init_states[0:2] # car1 position
        qpos[3:7] = init_states[2:6] # car1 quaternion
        qpos[14:16] = init_states[6:8] # car2 position
        qpos[17:21] = init_states[8:12] # car2 quaternion
        qpos[28:30] = init_states[12:14] # block position
        qpos[31:35] = init_states[14:18] # block quaternion
    ...
    def add_car_offset(self, car, additional = 0):
        c = car
        x = c[0]
        y = c[1]
        car_quat = R.from_quat(np.roll(c[-4:], -1))
        th = car_quat.as_euler('xyz', degrees=False)[2] 

        offset = 0.135 + additional
        x_off = x - offset*np.cos(th)
        y_off = y - offset*np.sin(th)

        c[0] = x_off
        c[1] = y_off
        # returns car_quat, car_theta for offset of 0.135m
        return c
    ...
    def _get_obs(self):
        ...
        car = position[:7].copy()
        car2 = position[14:21].copy()
        block = position[-7:].copy()
        car_vel = velocity[:6].copy()
        car2_vel = velocity[7:-6].copy()
        block_vel = velocity[-6:].copy()
        
        car_no_z = np.concatenate([car[:2], car[-4:]])
        car2_no_z = np.concatenate([car2[:2], car2[-4:]])
        block_no_z = np.concatenate([block[:2], block[-4:]])
        car_vel = np.concatenate([car_vel[:2], car_vel[-3:-1]])
        car2_vel = np.concatenate([car2_vel[:2], car2_vel[-3:-1]])
        block_vel = np.concatenate([block_vel[:2], block_vel[-3:-1]])

        car_no_z_offset = self.add_car_offset(car_no_z)
        car2_no_z_offset = self.add_car_offset(car2_no_z)

        return np.concatenate([np.array(car_no_z_offset),
                                np.array(car2_no_z_offset),
                                np.array(block_no_z),
                                np.array(car_vel), 
                                np.array(car2_vel),
                                np.array(block_vel)])
